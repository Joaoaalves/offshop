const COLUMNS: { key: string; header: string; get: (p: any) => string | number }[] = [
  ...
  { key: "storageCost",        header: "Custo Armazenamento",    get: (p) => p.storageCost ?? "" },
  { key: "priceWithTaxes",     header: "Preço c/ Impostos",      get: (p) => p.priceWithTaxes != null ? Number(p.priceWithTaxes).toFixed(2) : "" },
  { key: "unitPrice",          header: "Preço Unitário",         get: (p) => p.unitPrice != null ? Number(p.unitPrice).toFixed(2) : "" },
  ...
  { key: "volumeCm3",          header: "Volume (cm³)",           get: (p) => p.volumeCm3 ?? "" },
  { key: "weightKg",           header: "Peso (kg)",              get: (p) => p.weightKg ?? "" },
  { key: "chargeableWeightKg", header: "Peso Tarifável (kg)",    get: (p) => p.chargeableWeightKg ?? "" },
  { key: "minStockDays",       header: "Dias Mín. Estoque",      get: (p) => p.minStockDays ?? "" },
  { key: "stock_storage",      header: "Estoque Galpão",         get: (p) => p.stock?.storage ?? "" },
  { key: "stock_incoming",     header: "Estoque A Caminho",      get: (p) => p.stock?.incoming ?? "" },
  { key: "stock_damage",       header: "Estoque Avaria",         get: (p) => p.stock?.damage ?? "" },
