...
export async function POST(req: Request) {
  ...
  const lines = payload.map((line) => ({
    sku:      line.sku,
    storage:  line.deposits.find((d) => d.name === "Galpão"    && !d.ignore)?.balance ?? 0,
    incoming: line.deposits.find((d) => d.name === "A Caminho" && !d.ignore)?.balance ?? 0,
    damage:   line.deposits.find((d) => d.name === "Avaria"    && !d.ignore)?.balance ?? 0,
  }));
